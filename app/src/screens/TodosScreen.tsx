...
export function TodosScreen() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ margin: '14px 16px 0', background: colors.surfaceAlt, borderRadius: 14, padding: 14 }}>
        <div style={{ height: 8, borderRadius: 4, background: 'rgba(127,127,127,0.2)', overflow: 'hidden' }}>
          <div style={{ height: 8, borderRadius: 4, background: colors.accent, width: `${progress}%` }} />
        </div>
        <p style={{ margin: '8px 0 0', fontSize: 12, fontWeight: 500, color: colors.textMuted }}>
          {done} of {todos.length} done · {progress}%
        </p>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px' }}>
        <div style={{ flex: 1, border: `1px solid ${colors.border}`, background: colors.surface, borderRadius: 12, padding: '0 14px' }}>
          <input
        ...
        <IconButton glyph="✚" color={colors.accent} size={22} label="Add task" onPress={submit} />
      </div>
